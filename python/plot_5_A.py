...
for a in range(2,5):
    ...
    axes[a-2].plot(degree,countdata[0],'r',linewidth = linewidth,label = "Blind multiview",alpha=.5)
    blind = np.append(blind,np.sum(countdata[0]))
    smart = np.append(smart,np.sum(countdata[1]))
    axes[a-2].plot(degree,countdata[1],'g',linewidth = linewidth,label = "Smart Rotation")
