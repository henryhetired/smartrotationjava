...
#plt.show()
f, (ax,ax2) = plt.subplots(2,1,sharex=True,gridspec_kw = {'height_ratios':[15, 1]})
plt.hold(True)
ax.plot(np.arange(32)*0.5,coverage_optimal,'--',label="Smart rotation")
ax.plot(np.arange(32)*0.5,coverage_standard,'--',label="Equal spacing")
ax2.plot(np.arange(32)*0.5,coverage_optimal,'--',label="Smart rotation")
ax2.plot(np.arange(32)*0.5,coverage_standard,'--',label="Equal spacing")
ax2.set_ylim([0,0.01])
ax.set_ylim(0.85,1)
...
ax.legend()
ax.set_title("Imaging coverage over time")
plt.savefig(pathtotext+"figures/coverage_over_time.pdf",format="pdf",bbox_inches="tight")